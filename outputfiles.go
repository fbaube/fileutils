...
func (pCC *CheckedContent) NewOutputFiles(subdirSuffix string) (*OutputFiles, error) {
	...
	if subdirSuffix == "" {
		p.OutputDirPath = pCC.absFP.DirPath()
		return p, nil
	}
	// Transform the file name (an absolute filepath)
	// into a nearly-same directory name.
	var dn AbsFilePath
	sdn, ok := SU.DirNameFromFileName(pCC.String(), subdirSuffix)
...
func (pOF *OutputFiles) NewOutputExt(filext string) (*OutputFileExt, error) {
	...
	newpath = FP.Join(pOF.OutputDirPath.S(), pOF.pInputFile.absFP.BaseName()+filext)
